...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function == LossFunction.MSE:
            ...
            error = np.log(np.cosh(y - self.predict(x)))
            return np.mean(error)
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function is LossFunction.MSE:
            grad = (-2/l) * x_T.dot(y - x.dot(self.w))
            return grad
        elif self.loss_function is LossFunction.LogCosh:
            grad = 1/l * x_T.dot(np.tanh(x.dot(self.w) - y))
            return grad
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        l = y.size
        indexes = np.random.randint(0, l, self.batch_size)
        batch_x = x[indexes]
        batch_y = y[indexes]
        batch_x_T = batch_x.T
        if self.loss_function is LossFunction.MSE:
            grad = (-2/self.batch_size) * batch_x_T.dot(batch_y - batch_x.dot(self.w))
            return grad
        elif self.loss_function is LossFunction.LogCosh:
            grad = 1/self.batch_size * batch_x_T.dot(np.tanh(batch_x.dot(self.w) - batch_y))
            return grad
...
class MomentumDescent(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        nu = self.lr()
        old_w = self.w
        self.h = self.alpha * self.h + nu * gradient
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_k1_est = self.m / (1 - self.beta_1 ** (self.iteration + 1))
        v_k1_est = self.v / (1 - self.beta_2 ** (self.iteration + 1))
        old_w = self.w
        nu = self.lr()
        self.w = self.w - (nu / (np.sqrt(v_k1_est) + self.eps)) * m_k1_est
        self.iteration += 1
        return self.w - old_w
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        l2_gradient[len(l2_gradient) - 1] = 0
